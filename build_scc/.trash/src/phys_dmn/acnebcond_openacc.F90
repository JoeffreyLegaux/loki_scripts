...
SUBROUTINE ACNEBCOND_OPENACC (YDCST, YDRIP, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIA, KLEV, LDREDPR&
&, YDSTA, PAPHI, PAPHIF, PAPRSF, PCP, PR, PDELP, PRH, PBLH, PQ, PQI, PQL, PQW, PT, PNCV, PGM&
&, PTS, PQCS, PNEBCOND, PHCRICS, PRHOUT, PQSATS, PRMF, PQCS0, PNEBS0, YDSTACK)
...
IF (YDML_PHY_MF%YRPHY%LRKCDEV) THEN
  ...
  DO JLEV=1, KLEV
    ...
    ZESN=FOEW (PT (JLON, JLEV), 1._JPRB)*PRMF (JLON, JLEV)+FOEW&
    & (PT (JLON, JLEV), 0._JPRB)*(1._JPRB-PRMF (JLON, JLEV))
    ...
    ZDRHDZ=ZRHIN*YDCST%RG/(PT (JLON, JLEV)*YDCST%RD)*(ZEPSILO*(YDCST%RLVTT+PRMF (JLON&
    &, JLEV)*(YDCST%RLSTT-YDCST%RLVTT))/(YDCST%RCPD*PT (JLON, JLEV))-1._JPRB)
    ...
    PHCRICS (JLON, JLEV)=1.0_JPRB-0.5_JPRB*SQRT (2.0_JPRB*ZMESH*(ZSIGMA)*&
    &*2+(1.0_JPRB-ZTEST)*(ZZDIST2*ZDRHDZ)**2+ZTEST*ZZDIST2*ZSIGMAZ**2)
...
ENDSUBROUTINE ACNEBCOND_OPENACC
