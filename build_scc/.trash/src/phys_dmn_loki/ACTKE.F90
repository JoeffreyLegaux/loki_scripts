SUBROUTINE ACTKE (YDCST, YDLDDH, YDMDDH, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF,  &
& PDELP, PR, PT, PU, PV, PQ, PQICONV, PQLCONV, PLSCPE, PCD, PCH, PGZ0, PTS, PQS, PQICE, PQLI, PECT, PPRODTH, PNLAB, PNLABCVP,  &
& PKTROV, PKQROV, PKQLROV, PKUROV, PXTROV, PXUROV, PNBVNO, PNEBS, PQCS, PNEBS0, PQCS0, PCOEFN, PFECT, PFECTI, PECT1, PTPRDY,  &
& PEDR, YDDDH)
  ...
  REAL(KIND=JPRB) :: ZFPRTH(KLON, 0:KLEV), ZFPRDY(KLON, 0:KLEV), ZFDISS(KLON, 0:KLEV), ZFDIFF(KLON, 0:KLEV),  &
  & ZFCORTKE(KLON, 0:KLEV)
  REAL(KIND=JPRB) :: ZTPRTH(KLON, KLEV), ZTDISS(KLON, KLEV), ZTDIFF(KLON, KLEV), ZTCORTKE(KLON, KLEV), ZEDR(KLON, KLEV)
...
#include "aclender.intfb.h"
  ...
  & TSPHY=>YDML_PHY_MF%YRPHY2%TSPHY, LECTFL=>YDML_PHY_MF%YRPHY%LECTFL, RG=>YDCST%RG, LFLEXDIA=>YDLDDH%LFLEXDIA,  &
  ...
  IF (LECTFL) THEN
    ...
    CALL FL2HL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, PECT, ZECT, 1)
    IF (LMUSCLFA) THEN
      ZTABFL(:, :) = 0._JPRB
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZECT, 1, ZTABFL)
      DO JLEV=1,KLEV
        DO JLON=KIDIA,KFDIA
  ...
  IF (NLEND == 0) THEN
    CALL ACBL89(YDCST, YDML_PHY_MF%YRPHY, YDML_PHY_MF%YRPHY0, KIDIA, KFDIA, KLON, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF,  &
    & PT, ZECT, PQ, PQICE, PQLI, PNLAB, PNLABCVP, PGZ0, PTS, ZUSLE, ZLMECT, ZPHI3)
    
  ELSE IF (NLEND > 0) THEN
    CALL ACLENDER(YDCST, YDML_PHY_MF%YRPHY, YDML_PHY_MF%YRPHY0, KIDIA, KFDIA, KLON, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF,  &
    & PT, PQ, PQICE, PQLI, PNLAB, PNLABCVP, PLSCPE, PU, PV, ZECT, PGZ0, ZUSLE, ZLMECT, ZPHI3)
  END IF
  
  CALL ACTURB(YDCST, YDML_PHY_MF%YRPHY, YDML_PHY_MF%YRPHY0, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS,  &
  & PAPRSF, PR, PT, PU, PV, ZECT, PQ, LLCONV, PLSCPE, ZLMECT, ZPHI3, PCD, PCH, PGZ0, PTS, PQS, PQICE, PQLI, PKTROV, PKQROV,  &
  & PKQLROV, PKUROV, PNBVNO, ZPRODTH, PNEBS, PQCS, PCOEFN, ZKCLS, ZECTCLS)
  ...
  DO JLEV=KTDIAT,KLEV
    DO JLON=KIDIA,KFDIA
      ZPRODTH(JLON, JLEV) = ZPRODTH(JLON, JLEV) + RPRTH*MAX(0._JPRB, PPRODTH(JLON, JLEV))
    END DO
  END DO
  !- - - - - - - - - - - - - - - - - - - - - - - - - -
  ! 1.2 Calcul d'evolution de l'ECT
  !- - - - - - - - - - - - - - - - - - - - - - - - - -
  CALL ACEVOLET(YDCST, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV, PAPHI, PAPHIF, PAPRS, PAPRSF, PDELP, ZECT, PKUROV,  &
  & PR, PT, PU, PV, ZUSLE, ZPRODTH, ZKCLS, ZECTCLS, ZECT1, ZPRDY, ZDIFF, ZDISS)
  ...
  DO JLEV=KTDIAT,KLEV
    DO JLON=KIDIA,KFDIA
      ZEDR(JLON, JLEV) = MIN(100._JPRB, ZUSLE(JLON, JLEV)*RG)*(0.5_JPRB*(ZECT1(JLON, JLEV) + PECT(JLON, JLEV)))**1.5
    END DO
  END DO
  IF (LECTFL) THEN
    ! Passage de ZECT1 sur les niveaux pleins pour calculer la tendance sur les FL
    ! puis le flux
    CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZECT1, 1, PECT1)
    DO JLEV=KTDIAT,KLEV
      DO JLON=KIDIA,KFDIA
    ...
    CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZPRDY, 1, PTPRDY)
    ! EDR
    CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZEDR, 1, PEDR)
    IF (LMUSCLFA .or. LFLEXDIA) THEN
      ! Dissipation
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZDISS, 1, ZTDISS)
      ! Terme de diffusion
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZDIFF, 1, ZTDIFF)
      ! Production dynamique
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZPRDY, 1, PTPRDY)
      ! Production thermique
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZPRODTH, 1, ZTPRTH)
      ! Dissipation
      CALL HL2FL(KIDIA, KFDIA, KLON, 1, KLEV, PAPRS, PAPRSF, ZDISS, 1, ZTDISS)
      DO JLEV=KTDIAT,KLEV
        DO JLON=KIDIA,KFDIA
          ZTCORTKE(JLON, JLEV) =  &
          & ZDET(JLON, JLEV) - ZTPRTH(JLON, JLEV) - PTPRDY(JLON, JLEV) - ZTDIFF(JLON, JLEV) - ZTDISS(JLON, JLEV)
  ...
  ELSE
    ...
    IF (LMUSCLFA) THEN
      ...
      DO JLEV=KTDIAT,KLEV
        DO JLON=KIDIA,KFDIA
          ZTCORTKE(JLON, JLEV) =  &
          & ZDET(JLON, JLEV) - ZPRODTH(JLON, JLEV) - ZPRDY(JLON, JLEV) - ZDIFF(JLON, JLEV) - ZDISS(JLON, JLEV)
  ...
  IF (LFLEXDIA) THEN
    ZTABFL(:, :) = 0._JPRB
    ZTABFL(KIDIA:KFDIA, KTDIAT:KLEV) = PDELP(KIDIA:KFDIA, KTDIAT:KLEV)*PECT1(KIDIA:KFDIA, KTDIAT:KLEV)
    IF (LDDH_OMP) THEN
