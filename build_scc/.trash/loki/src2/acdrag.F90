...
#include "stack.h"
  ...
  & YDML_PHY_MF%YRPHY0%HOBST*PGETRL*PGWDCS))
  ...
  ! Compiler bug
  ZUSTAR = ZSUMU*COS(2.0_JPRB*PVRLDI) + ZSUMV*SIN(2.0_JPRB*PVRLDI)
  ZVSTAR = ZSUMU*SIN(2.0_JPRB*PVRLDI) - ZSUMV*COS(2.0_JPRB*PVRLDI)
  ZA = PVRLAN**2 + 0.5_JPRB*(4._JPRB*(1.0_JPRB - PVRLAN)*(1.0_JPRB + ZALP1*PVRLAN) + PVRLAN*LOG(1.0_JPRB / MAX(ZEPS1, PVRLAN)) &
  & *(1.0_JPRB + ZALP2*PVRLAN)) / YDCST%RPI
  ZD = 0.5_JPRB*(4._JPRB*(1.0_JPRB - PVRLAN)*(1.0_JPRB + ZDEL1*PVRLAN) - 3._JPRB*PVRLAN*LOG(1.0_JPRB / MAX(ZEPS1, PVRLAN)) &
  & *(1.0_JPRB + ZDEL2*PVRLAN)) / YDCST%RPI
  ...
  ZSTUS = YDML_PHY_MF%YRPHY0%GWDSE*PGWDCS**2*ZNFNO(JLON, KLEV)*PGETRL*ZUSUR
  ZSTVS = YDML_PHY_MF%YRPHY0%GWDSE*PGWDCS**2*ZNFNO(JLON, KLEV)*PGETRL*ZVSUR
  ...
  ZTEMPF = ZUSR / MAX(ZEPS5, ZTUNE*YDML_PHY_MF%YRPHY0%HOBST*PGETRL*PGWDCS*ZNFNO(JLON, KLEV))
  ...
  DO JLEV=0,KLEV
    !DEC$ IVDEP
    PRAPTRAJ(JLON, JLEV) = YDML_PHY_MF%YRPHY0%GWDSE*PGWDCS**2*ZNFNO(JLON, KLEV)*PGETRL
    IF (YDML_PHY_MF%YRPHY%LNEWD) THEN
  ...
  DO JLEV=KTDIA,KLEV
    ...
    ZRZ = (PAPRS(JLON, KLEV) - PAPRS(JLON, JLEV)) / MAX(ZEPS4, ZZBB*YDML_PHY_MF%YRPHY0%HOBST*PGETRL*PGWDCS)
  ...
  DO JLEV=KTDIA,KLEV
    ...
    ZALTI = MIN(1.0_JPRB, (PAPRS(JLON, KLEV) - PAPRSF(JLON, JLEV)) / ZOBST)
    ZFORCL = ZLIFT*PRCORI*(1.0_JPRB - ZALTI) / (1.0_JPRB + ZALTI*YDML_PHY_MF%YRPHY0%GWDPROF)
