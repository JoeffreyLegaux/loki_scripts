SUBROUTINE ACDIFV2_OPENACC (LDSFORCS, YDCST, YGFL, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIA, KLEV, PAPRS, PCFAQ, PCFAS, PCFAU,  &
& PCFASV, PCFBQ, PCFBS, PCFBU, PCFBV, PCFBSV, PKTROV, PKQROV, PKQLROV, PKUROV, PDSE, PQT, PU, PV, POID, PT, PQL, PQI, PRDELP,  &
& PCOEFA, PALPHA1, PLVT, PQICE, PSFSV, PFCS, PFEV, PFMDU, PFMDV, PTSN, PXHROV, PDIFSV, PDIFTQ, PDIFTS, PSTRTU, PSTRTV, PDIFTQL,  &
& PDIFTQI, PDIFCQ, PDIFCS, PSTRCU, PSTRCV, PSHEAR, YDSTACK)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - OUTPUT 2D .
...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PFEV
  REAL(KIND=JPRB), INTENT(IN) :: PSFSV(KLON, 1:YGFL%NGFL_EXT)
  REAL(KIND=JPRB), INTENT(IN) :: PFMDU
  REAL(KIND=JPRB), INTENT(IN) :: PFMDV
...
#include "fctdoi.func.h"
  ...
  PSTRTU(JLON, KLEV) = PFMDU
  PSTRTV(JLON, KLEV) = PFMDV
  ...
  IF (YDML_PHY_MF%YRARPHY%LMSE .or. LDSFORCS) THEN
    ...
    PDIFTQ(JLON, KLEV) = PFEV
    PDIFTS(JLON, KLEV) = PFCS*ZEXNER + PFEV*ZCPVMD*PTSN
  END IF
  ...
  IF (YDML_PHY_MF%YRARPHY%LMSE .or. LDSFORCS .or. YDML_PHY_MF%YRPHY%LEDMFI) THEN
    ZN1(JLON, KLEV) = ZN1(JLON, KLEV) + PXHROV*PCFAS(JLON, KLEV)*PDIFTS(JLON, KLEV)
    ZN2(JLON, KLEV) = ZN2(JLON, KLEV) + PXHROV*PCFAQ(JLON, KLEV)*PDIFTQ(JLON, KLEV)
    ...
    IF (YDML_PHY_MF%YRARPHY%LMDUST .and. YGFL%NGFL_EXT /= 0) THEN
      DO JGFL=1,YGFL%NGFL_EXT
        ...
        ZNSV1(JLON, KLEV, JGFL) = ZNSV1(JLON, KLEV, JGFL) + PXHROV*PCFASV(JLON, KLEV, JGFL)*PDIFSV(JLON, KLEV, JGFL)
