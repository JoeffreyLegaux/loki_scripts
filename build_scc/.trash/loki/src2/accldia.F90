...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PUCLS
  REAL(KIND=JPRB), INTENT(IN) :: PVCLS
  REAL(KIND=JPRB), INTENT(IN) :: POROG
  REAL(KIND=JPRB), INTENT(OUT) :: PUGST
  REAL(KIND=JPRB), INTENT(OUT) :: PVGST
  REAL(KIND=JPRB), INTENT(OUT) :: PBLH
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PDCAPE
  INTEGER(KIND=JPIM), INTENT(OUT) :: KCLPH
  ...
  IF ((LDXTGST .or. LDXXGST) .and. YDPHY2%LRAFTKE) THEN
    ...
    DO JLEV=KLEV,1,-1
      ZZ = PAPHIFM(JLON, JLEV) - POROG - YDPHY2%HTKERAF*YDCST%RG
    ...
    ! Turbulent gusts.
    ZVCLS = MAX(ZEPS, PUCLS**2 + PVCLS**2)
    ZALPHA = PTKE(JLON, JLEVM1)*(YDPHY2%HTKERAF*YDCST%RG + POROG - PAPHIFM(JLON, JLEVM1 + 1)) - PTKE(JLON, JLEVM1 + 1) &
    & *(YDPHY2%HTKERAF*YDCST%RG + POROG - PAPHIFM(JLON, JLEVM1))
    ...
    & PV(JLON, YDTOPH%NT950)**2))*MAX(0._JPRB, SIGN(1._JPRB, PCAPE - YDPHY2%GCAPERAF))*MIN(1._JPRB, SQRT(MAX(0._JPRB, PCAPE /  &
    & (5._JPRB*YDPHY2%GCAPERAF))))
    ...
    & YDPHY2%FACRAFDCAPE*SQRT(2._JPRB*MAX(0._JPRB, -PDCAPE)) / SQRT(ZVCLS)
    PUGST = ZALPHA*PUCLS
    PVGST = ZALPHA*PVCLS
  END IF
  ...
  IF (LDXCLP .and. .not.YDPHY%LCOEFKTKE) THEN
    ...
    IF (ILEVBI > 1 .and. ILEVBI < KLEV) THEN
      ZHBOT = (PAPHIFM(JLON, ILEVBI) - POROG) / YDCST%RG
      ZHTOP = (PAPHIFM(JLON, ILEVBI - 1) - POROG) / YDCST%RG
      PBLH = ZHBOT + (ZHTOP - ZHBOT) / (PTKE(JLON, ILEVBI - 1) - PTKE(JLON, ILEVBI))*(ZECTBLH - PTKE(JLON, ILEVBI))
    ELSE IF (ILEVBI == KLEV) THEN
      PBLH = (PAPHIFM(JLON, KLEV) - POROG) / YDCST%RG
    ELSE IF (ILEVBI == 0) THEN
      PBLH = (PAPHIFM(JLON, YDTOPH%NTCOET) - POROG) / YDCST%RG
    ELSE
      PBLH = (PAPHIFM(JLON, ILEVBI) - POROG) / YDCST%RG
    END IF
