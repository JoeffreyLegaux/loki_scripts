...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PCDMR
  REAL(KIND=JPRB), INTENT(IN) :: PCDNMR
  REAL(KIND=JPRB), INTENT(IN) :: PCDNH
  REAL(KIND=JPRB), INTENT(IN) :: PCH
  REAL(KIND=JPRB), INTENT(IN) :: PCPS
  REAL(KIND=JPRB), INTENT(IN) :: PDPHI
  REAL(KIND=JPRB), INTENT(IN) :: PDPHIT
  REAL(KIND=JPRB), INTENT(IN) :: PDPHIV
  REAL(KIND=JPRB), INTENT(IN) :: PQS
  REAL(KIND=JPRB), INTENT(IN) :: PSTAB
  ...
  REAL(KIND=JPRB), INTENT(OUT) :: PUCLS
  REAL(KIND=JPRB), INTENT(OUT) :: PVCLS
  REAL(KIND=JPRB), INTENT(OUT) :: PUCLN
  REAL(KIND=JPRB), INTENT(OUT) :: PVCLN
...
#include "fcttrm.func.h"
  ...
  ZBN = YDPHY0%VKARMN / SQRT(PCDN)
  ZBNH = YDPHY0%VKARMN*SQRT(PCDNMR) / PCDNH
  ZBD = YDPHY0%VKARMN / SQRT(PCD)
  ZBH = YDPHY0%VKARMN*SQRT(PCDMR) / PCH
  ZRU = PDPHIV / PDPHI
  ZRS = PDPHIT / PDPHI
  ZLOGU = LOG(1.0_JPRB + ZRU*(EXP(ZBN) - 1.0_JPRB))
  ZLOGS = LOG(1.0_JPRB + ZRS*(EXP(ZBNH) - 1.0_JPRB))
  ZCORU = PSTAB*ZRU*(ZBN - ZBD) + (1.0_JPRB - PSTAB)*LOG(1.0_JPRB + ZRU*(EXP(MAX(0.0_JPRB, ZBN - ZBD)) - 1.0_JPRB))
  ZCORS = PSTAB*ZRS*(ZBNH - ZBH) + (1.0_JPRB - PSTAB)*LOG(1.0_JPRB + ZRS*(EXP(MAX(0.0_JPRB, ZBNH - ZBH)) - 1.0_JPRB))
  
  !     INTERPOLATION DES COMPOSANTES DU VENT.
  !     INTERPOLATION OF WIND COMPONENTS.
  
  ZIV = MAX(0.0_JPRB, MIN(1.0_JPRB, (ZLOGU - ZCORU) / ZBD))
  PUCLS = PU(JLON, KLEV)*ZIV
  PVCLS = PV(JLON, KLEV)*ZIV
  
  !     INTERPOLATION DES COMPOSANTES DU VENT NEUTRE.
  !     INTERPOLATION OF NEUTRAL WIND COMPONENTS.
  
  ZIV = MAX(0.0_JPRB, ZLOGU / ZBD)
  PUCLN = PU(JLON, KLEV)*ZIV
  PVCLN = PV(JLON, KLEV)*ZIV
  ...
  IF (YDPHY%LBLVAR) THEN
    ...
    PTCLS = PT(JLON, KLEV)
  ELSE
    PQCLS = PQS + ZIV*(PQ(JLON, KLEV) - PQS)
    PTCLS = (PCPS*PTS + ZIV*(PCP(JLON, KLEV)*PT(JLON, KLEV) - PCPS*PTS + PDPHI) - PDPHIT) / (YDCST%RCPD + ZCPVMD*PQCLS)
  END IF
  ...
  PZPCLS = PAPRS(JLON, KLEV) + ZRS*(PAPRSF(JLON, KLEV) - PAPRS(JLON, KLEV))
  ZEW = FOEW(PTCLS, ZDELTA)
  PRHCLS = (PZPCLS*PQCLS*(YDCST%RETV + 1.0_JPRB)) / (ZEW*(1.0_JPRB + YDCST%RETV*PQCLS))
