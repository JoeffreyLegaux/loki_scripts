...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PDPHIU
  REAL(KIND=JPRB), INTENT(IN) :: PGZ0
  REAL(KIND=JPRB), INTENT(IN) :: PLSM
  REAL(KIND=JPRB), INTENT(IN) :: PQSATS
  REAL(KIND=JPRB), INTENT(IN) :: PRTI
  REAL(KIND=JPRB), INTENT(IN) :: PTS
  LOGICAL, INTENT(IN) :: LDHMT
  REAL(KIND=JPRB), INTENT(INOUT) :: PCD
  REAL(KIND=JPRB), INTENT(INOUT) :: PCDN
  REAL(KIND=JPRB), INTENT(INOUT) :: PCDROV
  REAL(KIND=JPRB), INTENT(INOUT) :: PCE
  REAL(KIND=JPRB), INTENT(OUT) :: PCEROV
  REAL(KIND=JPRB), INTENT(INOUT) :: PCH
  REAL(KIND=JPRB), INTENT(INOUT) :: PCHROV
  REAL(KIND=JPRB), INTENT(INOUT) :: PQCLS
  REAL(KIND=JPRB), INTENT(INOUT) :: PRHCLS
  REAL(KIND=JPRB), INTENT(INOUT) :: PTCLS
  REAL(KIND=JPRB), INTENT(INOUT) :: PUCLS
  REAL(KIND=JPRB), INTENT(INOUT) :: PVCLS
  REAL(KIND=JPRB), INTENT(INOUT) :: PURAF
  REAL(KIND=JPRB), INTENT(INOUT) :: PVRAF
...
#include "fcttrm.func.h"
  ...
  ZLSM = PLSM + (1.0_JPRB - PLSM)*MAX(0.0_JPRB, SIGN(1.0_JPRB, YDML_PHY_MF%YRPHY1%TMERGL - PTS))
  ...
  DO JJ=1,YDML_PHY_MF%YRPHY%NITERFL
    ...
    IF (ZDELTAU10N <= 16.8_JPRB) THEN
      PCDN = (-2.2261E-07_JPRB*ZDELTAU10N**3) + (1.3067E-05_JPRB*ZDELTAU10N**2) + (-1.2719E-04_JPRB*ZDELTAU10N) + 1.3013E-03_JPRB
    ELSE IF (ZDELTAU10N <= 50._JPRB) THEN
      PCDN = (4.2684E-09_JPRB*ZDELTAU10N**4) + (-4.8208E-07_JPRB*ZDELTAU10N**3) + (1.6212E-05_JPRB*ZDELTAU10N**2) +  &
    ...
    ZUSR2 = SQRT(PCDN)
    ZTSR2 = ZCHN / SQRT(PCDN)
    ZQSR2 = (ZCEFAC*ZCEN*(1.0_JPRB - ZICHCE) + ZCHN*ZICHCE) / SQRT(PCDN)
  ...
  PCD = ZLSM0*PCD + ZLSM1*ZUSRODU**2
  PCH = ZLSM0*PCH + ZLSM1*ZUSRODU*ZTSRODT
  PCE = ZLSM0*PCH + ZLSM1*(ZUSRODU*ZQSRODQ*(1.0_JPRB - ZICHCE) + PCH*ZICHCE)
  
  PCDROV = ZLSM0*PCDROV + ZLSM1*PCD*ZDU*PAPRS(JLON, KLEV)*PRTI
  PCHROV = ZLSM0*PCHROV + ZLSM1*PCH*ZDU*PAPRS(JLON, KLEV)*PRTI
  PCEROV = ZLSM0*PCHROV + ZLSM1*(PCE*ZDU*PAPRS(JLON, KLEV)*PRTI*(1.0_JPRB - ZICHCE) + PCHROV*ZICHCE)
  
  IF (LDHMT) THEN
    ZLMOUCLS = YDML_PHY_MF%YRPHY0%VKARMN*PDPHIU*(ZTSR*(1.0_JPRB + YDCST%RETV*ZQ) + YDCST%RETV*(ZT + YDCST%RTT)*ZQSR) / ((ZT +  &
    & YDCST%RTT)*(1.0_JPRB + YDCST%RETV*ZQ)*ZUSR*ZUSR)
    ZLMOTCLS = ZLMOUCLS*PDPHIT / PDPHIU
    ...
    END IF
    ZLOGHS10 = LOG(PDPHIU / 10._JPRB / YDCST%RG)
    ZDUCLS = ZDELTAU10N + ZUSR*(ZLOGHS10 - ZPSI_U) / YDML_PHY_MF%YRPHY0%VKARMN
    PUCLS = ZLSM0*PUCLS + ZLSM1*PU(JLON, KLEV)*ZDUCLS / ZDU
    PVCLS = ZLSM0*PVCLS + ZLSM1*PV(JLON, KLEV)*ZDUCLS / ZDU
    ZLOGHS10 = LOG(PDPHIT / 10._JPRB / YDCST%RG)
    ZDTCLS = ZDELTAT10N + ZTSR*(ZLOGHS10 - ZPSI_T) / YDML_PHY_MF%YRPHY0%VKARMN
    PTCLS = ZLSM0*PTCLS + ZLSM1*(ZDTCLS + PTS - PDPHIT / YDCST%RCPD)
    ZDQCLS = ZDELTAQ10N + ZQSR*(ZLOGHS10 - ZPSI_T) / YDML_PHY_MF%YRPHY0%VKARMN
    PQCLS = ZLSM0*PQCLS + ZLSM1*(ZDQCLS + PQSATS)
    IF (YDML_PHY_MF%YRPHY%LNEIGE) THEN
      ZDELTA = MAX(0.0_JPRB, SIGN(1.0_JPRB, YDCST%RTT - PTCLS))
    ELSE
      ZDELTA = 0.0_JPRB
    END IF
    ZPCLS = PAPRS(JLON, KLEV) + PDPHIT / (YDCST%RG*ZHT)*(PAPRSF(JLON, KLEV) - PAPRS(JLON, KLEV))
    ZEW = FOEW(PTCLS, ZDELTA)
    PRHCLS = ZLSM0*PRHCLS + ZLSM1*(ZPCLS*PQCLS*(YDCST%RETV + 1.0_JPRB)) / (ZEW*(1.0_JPRB + YDCST%RETV*PQCLS))
  END IF
  ...
  IF (YDML_PHY_MF%YRPHY2%LRAFTUR) THEN
    !DEC$ IVDEP
    ZSCRAF = (PCD*(PU(JLON, KLEV)**2 + PV(JLON, KLEV)**2) / MAX(ZEPS1, PUCLS**2 + PVCLS**2)) / (1.0_JPRB + (LOG(1.0_JPRB + PDPHIU &
    &  / YDML_PHY_MF%YRPHY2%GZ0RAF) / LOG(1.0_JPRB + PDPHIU / PGZ0))**2)
    ZSCRAF = 1.0_JPRB + YDML_PHY_MF%YRPHY2%FACRAF*SQRT(ZSCRAF)
    PURAF = ZLSM*PURAF + (1.0_JPRB - ZLSM)*ZSCRAF*PUCLS
    PVRAF = ZLSM*PVRAF + (1.0_JPRB - ZLSM)*ZSCRAF*PVCLS
  END IF
