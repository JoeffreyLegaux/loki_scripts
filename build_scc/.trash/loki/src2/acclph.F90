...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PTHETAVS
  INTEGER(KIND=JPIM), INTENT(OUT) :: KCLPH
  REAL(KIND=JPRB), INTENT(OUT) :: PCLPH
  REAL(KIND=JPRB), INTENT(OUT) :: PVEIN
  ...
  ZX_PREV = 0.0_JPRB
  ZLCH_PREV = ZVAL_LCH
  ZTHETAV(JLON, KLEV + 1) = MAX(PTHETAVS, PTHETAV(JLON, KLEV))
  ...
  DO JLEV=KLEV,KTDIA,-1
    ...
    !-------------------------------------------------
    !
    PCLPH = PCLPH + (ZPHI(JLON, JLEV) - ZPHI(JLON, JLEV + 1))*ZPSI
    !
    !-------------------------------------------------
  ...
  PCLPH = MAX(PCLPH, PAPHIF(JLON, KLEV) - PAPHI(JLON, KLEV))
  ...
  !cdir unroll=8
  DO JLEV=KLEV,KTDIA,-1
    ZPSI = MAX(0.0_JPRB, MIN(1.0_JPRB, (PCLPH + PAPHI(JLON, KLEV) - ZPHI(JLON, JLEV + 1)) / (ZPHI(JLON, JLEV) - ZPHI(JLON, JLEV + &
    &  1))))
    ZCLPU = ZPSI*PU(JLON, JLEV) + (1.0_JPRB - ZPSI)*ZCLPU
    ZCLPV = ZPSI*PV(JLON, JLEV) + (1.0_JPRB - ZPSI)*ZCLPV
    IBIN = NINT(ZPSI)
    KCLPH = IBIN*JLEV + (1 - IBIN)*KCLPH
  END DO
  
  !cdir unroll=8
  IC = 1
  DO JLEV=KLEV - 1,KTDIA,-1
    INDI = INT(MAX(0.0_JPRB, SIGN(1.0_JPRB, PCLPH + PAPHI(JLON, KLEV) - ZPHI(JLON, JLEV + 1))))
    ZVITM = ZVITM + INDI*SQRT(PU(JLON, JLEV)**2 + PV(JLON, JLEV)**2)
    IC = IC + INDI
  END DO
  
  PVEIN = PCLPH*(ZVITM / FLOAT(IC)) / YDCST%RG
  !-------------------------------------------------
