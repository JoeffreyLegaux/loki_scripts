...
#include "stack.h"
  ...
  ZZGKEF = YDML_PHY_MF%YRPHY0%ALPHAE*PKCLS
  ...
  PDIFF(JLON, KLEV - 1) = (ZPA(JLON, KLEV - 1)*(ZA(JLON, KLEV) - ZA(JLON, KLEV - 1)) - ZANKP1*(PECTCLS - ZA(JLON, KLEV)))*ZGSDP
  ! JLON
  ...
  ELSE
    ZDET(JLON, KLEV) =  &
    & (MIN(YDML_PHY_MF%YRPHY0%ECTMAX, MAX(YDML_PHY_MF%YRPHY0%ECTMIN, PECTCLS)) - PECT(JLON, KLEV)) / YDML_PHY_MF%YRPHY2%TSPHY
    ! JLON
  END IF
