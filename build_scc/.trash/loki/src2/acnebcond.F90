...
#include "acnebsm.intfb.h"
  ...
  IF (YDML_PHY_MF%YRPHY%LXRCDEV) THEN
    ...
    ZMESHEXP = (YDML_PHY_MF%YRPHY0%REFLRHC / (YDML_PHY_MF%YRPHY0%TEQH*PGM))**YDML_PHY_MF%YRPHY0%RHCEXPDX
  ...
  IF (YDML_PHY_MF%YRPHY%LSMGCDEV) THEN
    ...
    ZFACT = ZDCRI*(1.0_JPRB - YDML_PHY_MF%YRPHY0%GRHCMOD*EXP(-1.0_JPRB / (YDML_PHY_MF%YRPHY0%TEQH*PGM)))
    ZA = ZFACTA*ZFACT
    ZB = ZFACTB*ZFACT
    ZC = ZFACTC*ZFACT
  ...
  IF (YDML_PHY_MF%YRPHY%LRKCDEV) THEN
    ...
    DO JLEV=1,KLEV
      ...
      ZMESH = YDML_PHY_MF%YRPHY0%REFLRHC / (YDML_PHY_MF%YRPHY0%TEQH*PGM)
      ...
      ZTEST = MAX(0.0_JPRB, SIGN(1.0_JPRB, PAPHIF(JLON, JLEV) - PAPHI(JLON, KLEV) - YDCST%RG*PBLH))
  ...
  IF (YDML_PHY_MF%YRPHY%LSMGCDEV) THEN
    ...
    IF (YDML_PHY_MF%YRPHY%NSMTBOT == 0) THEN
      ZTKLEV = (PTS + PT(JLON, KLEV))*0.5_JPRB
    ELSE
      ZTKLEV = PT(JLON, KLEV)
    END IF
