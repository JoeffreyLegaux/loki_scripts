...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(OUT) :: PVISICLD
  REAL(KIND=JPRB), INTENT(OUT) :: PVISIHYD
  REAL(KIND=JPRB), INTENT(OUT) :: PMXCLWC
  ...
  PMXCLWC = PQL(JLON, ILEVB)*ZRHOAIR(JLON, ILEVB)*ZDELTA + PQL(JLON, ILEVH)*ZRHOAIR(JLON, ILEVH)*(1 - ZDELTA)
  ...
  PVISICLD = ZCONTRAST / (YDVISI%COEF_CM1*ZC1*ZC2*(ZLCONTENT**YDVISI%COEF_CM4) + YDVISI%COEF_IM1*ZICONTENT**YDVISI%COEF_IM2)
  ! conversion from km to m !
  PVISICLD = ZSCALE*MIN(PVISICLD, 20.0_JPRB)
  
  PVISIHYD = ZCONTRAST / (ZBETARAYL + YDVISI%COEF_RM1*ZRCONTENT**YDVISI%COEF_RM2 + YDVISI%COEF_SM1*ZSCONTENT**YDVISI%COEF_SM2 +  &
  & YDVISI%COEF_GM1*ZGCONTENT**YDVISI%COEF_GM2)
  ! conversion from km to m !
  PVISIHYD = ZSCALE*MIN(PVISIHYD, 20.0_JPRB)
