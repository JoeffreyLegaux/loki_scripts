...
#include "stack.h"
  ...
  REAL(KIND=JPRB), INTENT(IN) :: PCAPE
  ...
  LOGICAL, INTENT(OUT) :: LDLINOX
  REAL(KIND=JPRB), INTENT(OUT) :: PLIGH_TOT
  REAL(KIND=JPRB), INTENT(OUT) :: PLIGH_CTG
  REAL(KIND=JPRB), INTENT(OUT) :: PCTOPH
  REAL(KIND=JPRB), INTENT(OUT) :: PPRECMX
  REAL(KIND=JPRB), INTENT(OUT) :: PICE
  REAL(KIND=JPRB), INTENT(OUT) :: PCDEPTH
  REAL(KIND=JPRB), INTENT(OUT) :: PWMFU
  REAL(KIND=JPRB), INTENT(OUT) :: PCHARGE
...
#include "fcttre.func.h"
  ...
  ZAREA = 1.0E-6_JPRB*4.0_JPRB*YDCST%RPI*YDCST%RA*YDCST%RA*PGAW
  ...
  IF (YGFL%NCHEM /= 0 .and. YDEPHY%NLIMODE /= 4) THEN
    CALL ABOR1_OPENACC('CULIGHT: NCHEM /= 0 .and. NLIMODE /= 4 NOT SUPPORTED', YDSTACK=YLSTACK)
  END IF
  
  
  IF (LDCUM .and. KCTOP >= 1 .and. KCTOP <= KLEV) THEN
    ...
    ZBASHG = (PAPHIF(JL, KCBOT) - PAPHIF(JL, KLEV))*ZGEO2KM
    PPRECMX = MAXVAL(PFPLCL(JL, KCTOP:KCBOT) + PFPLCN(JL, KCTOP:KCBOT))
    
    ! naj is this cloud base <  4 km OK for TM5's and MOZART's parameterisations ?
    IF (ZBASHG < (4.0_JPRB / PPLRG) .and. PPRECMX > 1.E-10_JPRB) THEN
      ...
      IFREEZ15 = -1
      DO JK=KLEV,1,-1
        IF (PT(JL, JK) <= YDCST%RTT .and. IFREEZ == -1) IFREEZ = JK
        ! below 0C
        IF (PT(JL, JK) <= YDCST%RTT - 25.0_JPRB .and. IFREEZ25 == -1) IFREEZ25 = JK
        ! below -25C
        IF (PT(JL, JK) <= YDCST%RTT - 15.0_JPRB .and. IFREEZ15 == -1) IFREEZ15 = JK
        ! below -15C
      END DO
      
      ! Total cloud depth
      PCTOPH = MAX((PAPHI(JL, KCTOP) - PAPHI(JL, KCBOT))*ZGEO2KM, 0.0_JPRB)
      
      ! Cold cloud depth
      PCDEPTH = MAX((PAPHI(JL, KCTOP) - PAPHI(JL, IFREEZ))*ZGEO2KM, 0.0_JPRB)
      PCDEPTH = MIN(PCDEPTH, PCTOPH)
      ...
      IF (YDEPHY%NLIMODE <= 3 .or. YDEPHY%NLIMODE == 7) THEN
        ...
        DO JK=KCTOP,KCBOT
          IF (PMFU(JL, JK) > 0.0_JPRB) THEN
            ...
            PWMFU = PWMFU + PMFU(JL, JK)*(PAPHI(JL, JK - 1) - PAPHI(JL, JK)) / ZRHO
          END IF
        END DO
      ...
      IF (YDEPHY%NLIMODE <= 3) THEN
        ...
        DO JK=1,KLEV
          PICE = PICE + MAX(0.0_JPRB, PLU(JL, JK)*(1.0_JPRB - FOEALFCU(PT(JL, JK))))*(PAPH(JL, JK) - PAPH(JL, JK - 1))*Z1G
        END DO
      END IF
      
      IF (YDEPHY%NLIMODE == 6) THEN
        ILEVTOPG = MAX(IFREEZ25, KCTOP)
        ILEVBOTG = MIN(IFREEZ, KCBOT)
        
        ! Convective cloud base height.
        ZCBASEH = MAX(0.0_JPRB, (PAPHI(JL, KCBOT) - PAPHI(JL, KLEV))*ZGEO2KM)
        
        ! Convective cloud depth.
        ZCDEPTH = MAX(0.0_JPRB, (PAPHI(JL, KCTOP) - PAPHI(JL, KCBOT))*ZGEO2KM)
        ...
        DO JK=ILEVTOPG,ILEVBOTG
          ...
          END IF
          PCHARGE = PCHARGE + ZQGRAUP*(MAX(0.0_JPRB, PLU(JL, JK)) + ZQSNOW)*(PAPH(JL, JK) - PAPH(JL, JK - 1))*Z1G
        END DO
      ...
      IF (YDEPHY%NLIMODE == 1) THEN
        ...
        IF (PCDEPTH > 1.0_JPRB .and. (PCTOPH - PCDEPTH) > 0.5_JPRB .and. PWMFU > 0.0_JPRB) THEN
          IF (LDLAND) THEN
            PLIGH_TOT = 3.1E-01_JPRB*PICE*PWMFU*PCDEPTH
          ELSE
            PLIGH_TOT = 1.3E-02_JPRB*PICE*PWMFU*PCDEPTH
          END IF
        END IF
        
      ELSE IF (YDEPHY%NLIMODE == 2) THEN
        ! PL was commented out.
        IF (PCDEPTH > 1.0_JPRB .and. (PCTOPH - PCDEPTH) > 0.7_JPRB .and. PCTOPH > 5.0_JPRB .and. PWMFU > 0.05_JPRB) THEN
          IF (LDLAND) THEN
            PLIGH_TOT = 6.5E-01_JPRB*PICE*PWMFU*PCDEPTH
          ELSE
            PLIGH_TOT = 8.0E-02_JPRB*PICE*PWMFU*PCDEPTH
          END IF
        END IF
      ...
      ELSE IF (YDEPHY%NLIMODE == 3) THEN
        ...
        IF (PCDEPTH > 1.0_JPRB .and. (PCTOPH - PCDEPTH) > 0.7_JPRB .and. PCTOPH > 5.0_JPRB .and. PWMFU > 0.05_JPRB) THEN
          IF (LDLAND) THEN
            PLIGH_TOT = 4.5E-03_JPRB*(PICE*PCDEPTH)**2.0_JPRB*PWMFU**0.5_JPRB
          ELSE
            PLIGH_TOT = 6.6E-04_JPRB*(PICE*PCDEPTH)**2.0_JPRB*PWMFU**0.5_JPRB
          END IF
        END IF
        
      ELSE IF (YDEPHY%NLIMODE == 4) THEN
        ! TM5 Meijer 2001.
        IF (PCTOPH > 5.0_JPRB .and. IFREEZ25 > 0) THEN
          IF (LDLAND) THEN
            PLIGH_TOT = 5.0_JPRB*4.0E6_JPRB*PFPLCL(JL, KLEV)*1.0E-9_JPRB*YDCST%RDAYI
          ELSE
            PLIGH_TOT = 0.1_JPRB*(5.0_JPRB*4.0E6_JPRB*PFPLCL(JL, KLEV)*1.0E-9_JPRB*YDCST%RDAYI)
          END IF
        END IF
      ...
      ELSE IF (YDEPHY%NLIMODE == 5) THEN
        ...
        IF (IFREEZ > 0) THEN
          IF (LDLAND) THEN
            PLIGH_TOT = 3.44E-5_JPRB*PCTOPH**4.9_JPRB*(24.0_JPRB*60.0_JPRB / ZAREA)
          ELSE
            PLIGH_TOT = 6.4E-4_JPRB*PCTOPH**1.73_JPRB*(24.0_JPRB*60.0_JPRB / ZAREA)
          END IF
        END IF
        
      ELSE IF (YDEPHY%NLIMODE == 6) THEN
        ! New lightning parameterization (Ph. Lopez, 2015; flash density in flashes/km2/day).
        IF (ZCDEPTH > 4.0_JPRB .and. PCAPE > 175._JPRB .and. ZCBASEH > 0.1_JPRB .and. KCTOP < IFREEZ15 .and. PCDEPTH > 1.0_JPRB &
        & ) THEN
          PLIGH_TOT = 36.0_JPRB*MIN(1.0_JPRB, (SQRT(ZAREA) / 30.0_JPRB)**0.15_JPRB)*PCHARGE*SQRT(PCAPE)*MIN(ZCBASEH, ZHBMAX)**2
        END IF
        
      ELSE IF (YDEPHY%NLIMODE == 7) THEN
        ! Grewe et al. (2001).
        ! Flash rate in flashes/min  -> convert to flashes/(day*km2).
        IF (PWMFU > 0.0_JPRB) THEN
          PLIGH_TOT = 1.54E-5_JPRB*(PWMFU*SQRT(PCTOPH*1000.0_JPRB))**4.9_JPRB*(1440.0_JPRB / ZAREA)
        END IF
      ...
      ELSE IF (YDEPHY%NLIMODE == 8) THEN
        ...
        IF (ZPREC_CV > 7.0_JPRB) THEN
          IF (LDLAND) THEN
            PLIGH_CTG = 3.75E-2_JPRB - 4.76E-2_JPRB*ZPREC_CV + 5.41E-3_JPRB*ZPREC_CV**2 + 3.21E-4_JPRB*ZPREC_CV**3 -  &
            & 2.93E-6_JPRB*ZPREC_CV**4
          ELSE
            PLIGH_CTG = 5.23E-2_JPRB - 4.80E-2_JPRB*ZPREC_CV + 5.45E-3_JPRB*ZPREC_CV**2 + 3.68E-5_JPRB*ZPREC_CV**3 -  &
            & 2.42E-7_JPRB*ZPREC_CV**4
          END IF
          PLIGH_CTG = MAX(PLIGH_CTG, 0.0_JPRB)
          ! Cloud-to-ground flash rate in flashes/min  -> convert to flashes/(day*km2).
          PLIGH_CTG = PLIGH_CTG*(1440.0_JPRB / ZAREA_REF)
          ! Convert to total lightning using Price and Rind (1994).
          ZCOLDDPT = MAX(PCDEPTH, 5.5_JPRB)
      ...
      IF (YDEPHY%NLIMODE /= 8) THEN
        IF (PCDEPTH > 5.5_JPRB .and. PCDEPTH < 14.0_JPRB) THEN
          ZPCTG = 1.0_JPRB / (0.021_JPRB*(PCDEPTH**4.0_JPRB) - 0.648_JPRB*(PCDEPTH**3.0_JPRB) + 7.49_JPRB*(PCDEPTH**2.0_JPRB) -  &
          & 36.54_JPRB*PCDEPTH + 64.09_JPRB)
          PLIGH_CTG = ZPCTG*PLIGH_TOT
        ELSE
          PLIGH_CTG = PLIGH_TOT
        END IF
      END IF
