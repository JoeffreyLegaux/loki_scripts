...
#include "stack.h"
  ...
  IF (YDML_PHY_MF%YRPHY%LRNUMX .and. YDML_PHY_MF%YRPHY%LACPANMX) THEN
    PCLCH = 1._JPRB - PNEB(JLON, KTDIA)
  ELSE
    PCLCH = PNEB(JLON, KTDIA)
  END IF
  ...
  DO JLEV=KTDIA + 1,KLEV - 1
    ...
    IF (YDML_PHY_MF%YRPHY%LRNUMX .and. YDML_PHY_MF%YRPHY%LACPANMX) THEN
      ...
      ELSE IF (ZH >= YDML_PHY_MF%YRPHY2%HWMOHIGH) THEN
        PCLCM = 1._JPRB - PNEB(JLON, JLEV)          ! complement of medium cloud cover
      ELSE IF (ZL >= YDML_PHY_MF%YRPHY2%HWMOLOW) THEN
        PCLCM = PCLCM*ZOVLP
      ELSE IF (ZH >= YDML_PHY_MF%YRPHY2%HWMOLOW) THEN
        PCLCL = 1._JPRB - PNEB(JLON, JLEV)          ! complement of low cloud cover
      ELSE
        PCLCL = PCLCL*ZOVLP
      END IF
    ELSE
      ...
      ZH = (PAPHI(JLON, JLEV - 2) - PAPHI(JLON, KLEV)) / YDCST%RG
      IF (ZL >= YDML_PHY_MF%YRPHY2%HWMOHIGH) THEN
        PCLCH = PCLCH + (PNEB(JLON, JLEV) - ZOVLP)*(1._JPRB - PCLCH) / (1._JPRB - PNEB(JLON, JLEV - 1))
      ELSE IF (ZH >= YDML_PHY_MF%YRPHY2%HWMOHIGH) THEN
        PCLCM = PNEB(JLON, JLEV)          ! initialize medium cloud cover
      ELSE IF (ZL >= YDML_PHY_MF%YRPHY2%HWMOLOW) THEN
        PCLCM = PCLCM + (PNEB(JLON, JLEV) - ZOVLP)*(1._JPRB - PCLCM) / (1._JPRB - PNEB(JLON, JLEV - 1))
      ELSE IF (ZH >= YDML_PHY_MF%YRPHY2%HWMOLOW) THEN
        PCLCL = PNEB(JLON, JLEV)          ! initialize low cloud cover
      ELSE
        PCLCL = PCLCL + (PNEB(JLON, JLEV) - ZOVLP)*(1._JPRB - PCLCL) / (1._JPRB - PNEB(JLON, JLEV - 1))
      END IF
    END IF
