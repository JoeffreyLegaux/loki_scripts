...
#include "abor1.intfb.h"
  ...
  temp (REAL (KIND=JPRB), Z1MU0M, (YDGEOMETRY%YRDIM%NPROMA))
  temp (REAL (KIND=JPRB), Z2MU0M, (YDGEOMETRY%YRDIM%NPROMA))
  temp (REAL (KIND=JPRB), ZCOS, (YDGEOMETRY%YRDIM%NPROMA))
  temp (REAL (KIND=JPRB), ZSIN, (YDGEOMETRY%YRDIM%NPROMA))
  ...
  & YDMODEL%YRML_GCONF%YRRIP%RCODEC*YDMODEL%YRML_GCONF%YRRIP%RCOVSR*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
  & YDMODEL%YRML_GCONF%YRRIP%RCODEC*YDMODEL%YRML_GCONF%YRRIP%RSIVSR*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON), 0.0_JPRB))
  
  IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
    !DEC$ IVDEP
    PMU0N(JLON) = MIN(1.0_JPRB, MAX(YDMODEL%YRML_GCONF%YRRIP%RSIDECN*PGEMU(JLON) -  &
    & YDMODEL%YRML_GCONF%YRRIP%RCODECN*YDMODEL%YRML_GCONF%YRRIP%RCOVSRN*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
    & YDMODEL%YRML_GCONF%YRRIP%RCODECN*YDMODEL%YRML_GCONF%YRRIP%RSIVSRN*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON), 0.0_JPRB))
  ELSE
    PMU0N(JLON) = 0.0
  END IF
  ...
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LRAYLU) THEN
    ...
    & YDMODEL%YRML_GCONF%YRRIP%RCODECLU*YDMODEL%YRML_GCONF%YRRIP%RCOVSRLU*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
    & YDMODEL%YRML_GCONF%YRRIP%RCODECLU*YDMODEL%YRML_GCONF%YRRIP%RSIVSRLU*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON), 0.0_JPRB)
  END IF
  ...
  & .not.YDMODEL%YRML_PHY_MF%YRPHY%LRMU0M) THEN
    ...
    & YDMODEL%YRML_PHY_RAD%YRERIP%RCODECM*YDMODEL%YRML_PHY_RAD%YRERIP%RCOVSRM*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
    & YDMODEL%YRML_PHY_RAD%YRERIP%RCODECM*YDMODEL%YRML_PHY_RAD%YRERIP%RSIVSRM*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON),  &
    & 0.0_JPRB)
  ...
  ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LMPHYS .and. YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM .and. YDMODEL%YRML_PHY_MF%YRPHY%LRMU0M) THEN
    !DEC$ IVDEP
    Z1MU0M(JLON) = MAX(YDMODEL%YRML_PHY_RAD%YRERIP%RSIDECM*PGEMU(JLON) -  &
    & YDMODEL%YRML_PHY_RAD%YRERIP%RCODECM*YDMODEL%YRML_PHY_RAD%YRERIP%RCOVSRM*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
    & YDMODEL%YRML_PHY_RAD%YRERIP%RCODECM*YDMODEL%YRML_PHY_RAD%YRERIP%RSIVSRM*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON),  &
    & 0.0_JPRB)
    Z2MU0M(JLON) = 0.5_JPRB*(MAX(YDMODEL%YRML_GCONF%YRRIP%RSIDECF*PGEMU(JLON) -  &
    & YDMODEL%YRML_GCONF%YRRIP%RCODECF*YDMODEL%YRML_GCONF%YRRIP%RCOVSRF*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZCOS(JLON) +  &
    & YDMODEL%YRML_GCONF%YRRIP%RCODECF*YDMODEL%YRML_GCONF%YRRIP%RSIVSRF*SQRT(1.0_JPRB - PGEMU(JLON)**2)*ZSIN(JLON), 0.0_JPRB) +  &
    & PMU0(JLON))
  ...
  ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LMPHYS .and. YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM15) THEN
    CALL ABOR1_OPENACC('RADIATION SCHEME FROM CYCLE 15 NO LONGER AVAILABLE', YDSTACK=YLSTACK)
  ELSE
    PMU0M(JLON) = 0.0_JPRB
  END IF
  ...
  IF (LLCOMPUTE_CVGQ) THEN
    ...
    IF (YDMODEL%YRML_DYN%YRDYN%NCOMP_CVGQ == 0 .or. YDMODEL%YRML_DYN%YRDYN%NCOMP_CVGQ == 1) THEN
      ...
      ELSE
        ...
        DO JLEV=1,YDGEOMETRY%YRDIMV%NFLEVG - 1
          ZED = 0.5_JPRB*PEVEL0(JLON, JLEV)
          
          PCVGQ(JLON, JLEV) = PCVGQ(JLON, JLEV) + (PQT0(JLON, JLEV) - PQT0(JLON, JLEV + 1))*ZED*PRDELP0(JLON, JLEV)
          
          PCVGQ(JLON, JLEV + 1) = PCVGQ(JLON, JLEV + 1) + (PQT0(JLON, JLEV) - PQT0(JLON, JLEV + 1))*ZED*PRDELP0(JLON, JLEV + 1)
        END DO
      END IF
