...
from loki import (
    ...
    sanitise_imports, backend, InlineCall, Comment
)
                ...
                if not s.type.pointer and not s.type.parameter : #if pt, 
                    if s.name not in routine_arg:
                        ...
                        for shape in s.shape:
                            ...
                            if (not isinstance(shape, symbols.IntLiteral)):
                                if isinstance(shape, symbols.Scalar) or isinstance(shape, symbols.DeferredTypeSymbol) :
                        ...
                        if not_cste: 
                            ...
                            for shape in s.shape:
                                new_s=new_s+ backend.fgen(shape)+', '
                            new_s=new_s[:-2]
                            new_s=new_s+'))'
...
#    if verbose: print("horizontal size = ", horizontal.size)
    ...
    if len(hor_lst)>1:
        print(colored(f'multiple horizontal sizes are used : {hor_lst}', "red"))
        ...
        for call in FindInlineCalls().visit(assign):
            if (call.name=="SUM"):
                if len(call.parameters)>1:
                    ...
                    if len(arg.shape)==1:
                        if arg.shape[0].name in horizontal_size:
...
def get_dim_pt(routine, tmp_pt, tmp_target, horizontal_size):
    ...
    for assign in FindNodes(Assignment).visit(routine.body):
        ...
        lhs=list(lhs)
        if assign.ptr and lhs[0].name in [ptr.name for ptr in tmp_pt]:
            if isinstance(assign.rhs, InlineCall) and assign.rhs.name == 'NULL':
                #assign_map[assign] = Comment(text='! [ Commented by Loki ] '+backend.fgen(assign))
                assign_map[assign] = Intrinsic(f'nullptr({lhs[0].name})')
            ...
            #for target in tmp_target:
                   ...
                   #print("is tARGET ! ")
                   #break
                target = next(tgt for tgt in tmp_target if tgt.name == rhs[0].name)
                if target:
#TODO: mv target.dimensions[0]=='KPROMA' in find_pt
                    ...
                    #print("dims ? ", target.dimensions[0])
                    if target.dimensions[0] in horizontal_size: 
...
#*********************************************************
#*********************************************************
def scc_transform_routine(routine, lst_horizontal_size, lst_horizontal_idx, lst_horizontal_bounds, true_symbols, false_symbols, pathw = None, FieldAPI_pointers = {}, is_node = False):
    #----------------------------------------------
    #transformations:
    #----------------------------------------------
    horizontal_size=get_horizontal_size(routine, lst_horizontal_size)
    do_resolve_associates(routine)
    # Already enforced at the higher level
    #logical.transform_subroutine(routine, true_symbols, false_symbols)
    logical.transform_subroutine(routine, [], ['LHOOK'])
    
    end_index, begin_index, new_range=ExplicitArraySyntaxes.ExplicitArraySyntaxes(routine, lst_horizontal_size, lst_horizontal_bounds, FieldAPI_pointers)
    ...
    # Do not demote if node from a routine : the variable will be turned into FieldAPI, needs to stay an array !
    if horizontal_size != [] and not is_node :
...
#    rm_sum(routine), horizontal_size)
    ...
    tmp_pt_klon=get_dim_pt(routine, tmp_pt, tmp_target, lst_horizontal_size)
