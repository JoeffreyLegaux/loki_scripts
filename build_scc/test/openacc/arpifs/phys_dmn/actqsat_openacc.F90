...
#include "fcttrm.func.h"
  ...
  DO JLEV=KTDIA,KLEV
    ...
    DO JIT=1,YDPHY%NBITER
      ...
      PGEOSLC(JLON, JLEV) = PTW(JLON, JLEV)*(ZCP + ZLH*ZDQW) / (1.0_JPRB + ZLH*PQW(JLON, JLEV) / (PTW(JLON, JLEV) &
      & *YDCST%RD*(1.0_JPRB + YDCST%RETV*PQW(JLON, JLEV))))
    ...
    PRH(JLON, JLEV) =  &
    & (PQ(JLON, JLEV)*(1.0_JPRB + YDCST%RETV*PQSAT(JLON, JLEV))) / (PQSAT(JLON, JLEV)*(1.0_JPRB + YDCST%RETV*PQ(JLON, JLEV)))
