...
#include "wrscmr.intfb.h"
  ...
  IF (.not.YDPHY%LECTREP) THEN
    ...
    PECTCLS(JLON) = MAX(YDPHY0%ECTMIN, YDPHY0%AECLS3*ZUSTAR**2 + YDPHY0%AECLS4*ZWSTAR**2*(1.0_JPRB - ZSTAB))
  ...
  IF (YDPHY%LNEBECT) THEN
    ...
    DO JLEV=KLEV,KTDIAN,-1
      ...
      ZDQW = PQV(JLON, IJLEVM1) - PQV(JLON, IJLEVP1)*REAL(INIV, kind=JPRB) - PQS(JLON)*REAL(1 - INIV, kind=JPRB) + PQLI(JLON,  &
      & IJLEVM1) + PQICE(JLON, IJLEVM1) - PQLI(JLON, IJLEVP1) - PQICE(JLON, IJLEVP1)
      ZDT = PT(JLON, IJLEVM1) - PT(JLON, IJLEVP1)*REAL(INIV, kind=JPRB) - PTS(JLON)*REAL(1 - INIV, kind=JPRB)
