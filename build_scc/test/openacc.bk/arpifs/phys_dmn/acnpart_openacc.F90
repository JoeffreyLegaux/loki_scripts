...
#include "stack.h"
  ...
  REAL(KIND=JPRB), OPTIONAL, INTENT(OUT) :: PCLCC(KLON)
  REAL(KIND=JPRB), OPTIONAL, INTENT(IN) :: PNEBC(KLON, KLEV)
  REAL(KIND=JPRB), OPTIONAL, INTENT(OUT) :: PTOPC(KLON)
...
#include "acnpart_conv_base_top_openacc.intfb.h"
  ...
  ELSE
    ...
    CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, KTDIA, YDML_PHY_MF%YRPHY2%NTSHM,  &
    & PDECRD, PNEB, PAPRSF, PCLCH, YDSTACK=YLSTACK)
    ! high
    CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, YDML_PHY_MF%YRPHY2%NTSHM + 1,  &
    & YDML_PHY_MF%YRPHY2%NTSML, PDECRD, PNEB, PAPRSF, PCLCM, YDSTACK=YLSTACK)
    ! medium
    CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, YDML_PHY_MF%YRPHY2%NTSML + 1,  &
    & KLEV, PDECRD, PNEB, PAPRSF, PCLCL, YDSTACK=YLSTACK)
    ! low
    
  END IF
  
  ! total cloud cover
  CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, KTDIA, KLEV, PDECRD, PNEB, PAPRSF,  &
  & PCLCT, YDSTACK=YLSTACK)
  
  ! convective cloud cover
  IF (PRESENT(PCLCC)) THEN
    CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, KTDIA, KLEV, PDECRD, PNEBC,  &
    & PAPRSF, PCLCC, YDSTACK=YLSTACK)
  END IF
  ...
  IF (YDML_PHY_MF%YRPHY%LRNUMX .and. (YDML_PHY_MF%YRPHY%LACPANMX .or. YDML_PHY_MF%YRPHY%LRNUEXP)) THEN
    ...
    CALL ACNPART_CLOUD_COVER_OPENACC(YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, ZDECRDRED, ZWMXOV, KTDIA, KLEV, PDECRD, PNEB,  &
    & PAPRSF, PCLCT_RAD, YDSTACK=YLSTACK)
  ELSE
  ...
  IF (PRESENT(PTOPC) .and. YDML_PHY_MF%YRPHY%LPTOPC) THEN
    CALL ACNPART_CONV_BASE_TOP_OPENACC(YDCST, YDML_PHY_MF, KIDIA, KFDIA, KLON, KLEV, PNEBC, PAPRSF, PAPHIF, PTOPC,  &
    & YDSTACK=YLSTACK)
  END IF
