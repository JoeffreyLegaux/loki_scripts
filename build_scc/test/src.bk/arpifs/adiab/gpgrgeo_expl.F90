!OCL  NOEVAL
SUBROUTINE GPGRGEO_EXPL(YDGEOMETRY, KPROMA, KST, KEND, KFLEV, PRT, PRTL, PRTM, PLNPR,     &
...
IF (LLNHEE) THEN
  ...
  DO JLEV=1,KFLEV
    ZNH1L(KST:KEND,JLEV)=-PRT(KST:KEND,JLEV)*PQCHAL(KST:KEND,JLEV)
    ZNH1M(KST:KEND,JLEV)=-PRT(KST:KEND,JLEV)*PQCHAM(KST:KEND,JLEV)
  ENDDO
...
IF(YDGEOMETRY%YRVERT_GEOM%YRCVER%LVERTFE) THEN
  ...
  CALL VERDISINT(YDGEOMETRY%YRVERT_GEOM%YRVFE, YDGEOMETRY%YRVERT_GEOM%YRCVER, 'IBOT', '11', KPROMA, KST, KEND, KFLEV, ZINL, ZGPHL)
  CALL VERDISINT(YDGEOMETRY%YRVERT_GEOM%YRVFE, YDGEOMETRY%YRVERT_GEOM%YRCVER, 'IBOT', '11', KPROMA, KST, KEND, KFLEV, ZINM, ZGPHM)
