...
REAL(KIND=JPRB) :: ZPHI(KPROMA,0:KFLEV+1),ZOUT(KPROMA,KFLEV+1)
  ...
  DO JLEV=1,KFLEV
    DO JLON=KST,KEND
      ZPHI(JLON,JLEV)=-PR(JLON,JLEV)*PT(JLON,JLEV)&
       & *PLNPR(JLON,JLEV)*YDVGEOM%YRVETA%VFE_RDETAH(JLEV)  
    ENDDO
  ENDDO

  ZPHI(KST:KEND,0)=0.0_JPRB
  ZPHI(KST:KEND,KFLEV+1)=0.0_JPRB
  CALL VERDISINT(YDVGEOM%YRVFE, YDVGEOM%YRCVER, 'IBOT', '11', KPROMA, KST, KEND, KFLEV, ZPHI, ZOUT)
