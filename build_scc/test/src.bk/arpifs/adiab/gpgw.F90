...
IF (LLVFE) THEN
  ...
  IF (LDGDWI) THEN
    ...
    CALL VERDISINT(YDGEOMETRY%YRVERT_GEOM%YRVFE,YDGEOMETRY%YRVERT_GEOM%YRCVER,'ITOP','00',KPROMA,KST,KEND,KFLEV,ZIN,ZGWH)
  ELSEIF (LDNHDYN) THEN
    ZIN(KST:KEND,0)      = ZGDW(KST:KEND,1)
    ZIN(KST:KEND,KFLEV+1)= ZGDW(KST:KEND,KFLEV) ! not applied with INGW
    CALL VERDISINT(YDGEOMETRY%YRVERT_GEOM%YRVFE,YDGEOMETRY%YRVERT_GEOM%YRCVER,'INGW','00',KPROMA,KST,KEND,KFLEV,ZIN,ZGWH)
  ELSE
    ZIN(KST:KEND,0) = 0.0_JPRB
    ZIN(KST:KEND,KFLEV+1) = 0.0_JPRB
    CALL VERDISINT(YDGEOMETRY%YRVERT_GEOM%YRVFE,YDGEOMETRY%YRVERT_GEOM%YRCVER,'IBOT','11',KPROMA,KST,KEND,KFLEV,ZIN,ZGWH)
  ENDIF

  DO JLEV=1,KFLEV
    DO JROF=KST,KEND
      PGWF(JROF,JLEV) = ZGWH(JROF,JLEV)+PGWH(JROF,KFLEV)
    ENDDO
  ENDDO
ELSE
