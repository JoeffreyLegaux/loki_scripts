...
#include "fcttrm.func.h"
  ...
  DO JLEV=KTDIA,KLEV
    IF (YDPHY%LNEIGE) THEN
      ZZPN = PLSM(JLON)*PNEIJ(JLON) + (1.0_JPRB - PLSM(JLON))*MAX(0.0_JPRB, SIGN(1.0_JPRB, YDPHY1%TMERGL - PTS(JLON)))
    ELSE
      ZZPN = 0.0_JPRB
    END IF
    ...
    ZZDLLW = (1.0_JPRB - ZZPN)*PCHROV(JLON)*((PQS(JLON) - PQ(JLON, KLEV))*ZCPVMW + FOLH(PTS(JLON), 0.0_JPRB)*PDQSTS(JLON))
    
    ! Derivee du flux de chaleur latente sur glace
    ZZDLLS = PCHROV(JLON)*ZZPN*((PQS(JLON) - PQ(JLON, KLEV))*ZCPVMS + FOLH(PTS(JLON), 1.0_JPRB)*PDQSTS(JLON))
