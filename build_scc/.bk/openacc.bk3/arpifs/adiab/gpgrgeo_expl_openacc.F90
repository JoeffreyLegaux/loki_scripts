...
#include "stack.h"
  ...
  temp (REAL (KIND=JPRB), ZGPHL, (KPROMA, KFLEV + 1))
  temp (REAL (KIND=JPRB), ZGPHM, (KPROMA, KFLEV + 1))
...
#include "verdisint_openacc.intfb.h"
  ...
  IF (LLNHEE) THEN
    ...
    DO JLEV=KFLEV,1,-1
      PHIHL(JLON, JLEV - 1) = PHIHL(JLON, JLEV) + PLNPR(JLON, JLEV)*PRNHPPI(JLON, JLEV)*(PRTL(JLON, JLEV) + ZNH1L(JLON, JLEV)) +  &
      & PLNPRL_DER(JLON, JLEV)*PRNHPPI(JLON, JLEV)*PRT(JLON, JLEV)
      PHIHM(JLON, JLEV - 1) = PHIHM(JLON, JLEV) + PLNPR(JLON, JLEV)*PRNHPPI(JLON, JLEV)*(PRTM(JLON, JLEV) + ZNH1M(JLON, JLEV)) +  &
      & PLNPRM_DER(JLON, JLEV)*PRNHPPI(JLON, JLEV)*PRT(JLON, JLEV)
    END DO
  ...
  IF (LLNHEE) THEN
    DO JLEV=1,KFLEV
      ZINL(JLON, JLEV) = (-PLNPR(JLON, JLEV)*PRNHPPI(JLON, JLEV)*(PRTL(JLON, JLEV) + ZNH1L(JLON, JLEV)) - PLNPRL_DER(JLON, JLEV) &
      & *PRNHPPI(JLON, JLEV)*PRT(JLON, JLEV))*YDGEOMETRY%YRVERT_GEOM%YRVETA%VFE_RDETAH(JLEV)
      ZINM(JLON, JLEV) = (-PLNPR(JLON, JLEV)*PRNHPPI(JLON, JLEV)*(PRTM(JLON, JLEV) + ZNH1M(JLON, jLEV)) - PLNPRM_DER(JLON, JLEV) &
      & *PRNHPPI(JLON, JLEV)*PRT(JLON, JLEV))*YDGEOMETRY%YRVERT_GEOM%YRVETA%VFE_RDETAH(JLEV)
    END DO
  ELSE
    DO JLEV=1,KFLEV
  ...
  CALL VERDISINT_OPENACC(YDGEOMETRY%YRVERT_GEOM%YRVFE, YDGEOMETRY%YRVERT_GEOM%YRCVER, 'IBOT', '11', KPROMA, KST, KEND, KFLEV,  &
  & ZINL, ZGPHL, YDSTACK=YLSTACK)
  CALL VERDISINT_OPENACC(YDGEOMETRY%YRVERT_GEOM%YRVFE, YDGEOMETRY%YRVERT_GEOM%YRCVER, 'IBOT', '11', KPROMA, KST, KEND, KFLEV,  &
  & ZINM, ZGPHM, YDSTACK=YLSTACK)
  
  DO JLEV=1,KFLEV
    PHIFL(JLON, JLEV) = ZGPHL(JLON, JLEV) + POROGL(JLON)
    PHIFM(JLON, JLEV) = ZGPHM(JLON, JLEV) + POROGM(JLON)
  END DO
