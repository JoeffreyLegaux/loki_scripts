...
#include "fzh.func.h"
  ...
  IF (YDPHY%LNEIGE) THEN
    IF (YDPHY%LSNV) THEN
      ...
      PGZ0(JLON) = SQRT((1.0_JPRB - PNEIJV(JLON))*PGZ0F(JLON)**2 + PNEIJV(JLON)*(Z0CR**2 + PGZ0RLF(JLON)**2))
    ELSE
      PNEIJ(JLON) = PLSM(JLON)*PSNS(JLON) / (PSNS(JLON) + YDPHY1%WCRIN)
      PVEG(JLON) = PVEG0(JLON)
      PGZ0(JLON) =  &
      & PGZ0F(JLON) + (Z0CR - PGZ0F(JLON))*PLSM(JLON)*PSNS(JLON) / (PSNS(JLON) + YDPHY1%WCRIN*(1.0_JPRB + ZUZ0CN*PGZ0F(JLON)))
    END IF
  ...
  IF (LDZ0H) THEN
    ...
    IF (YDPHY%LNEIGE) THEN
      IF (YDPHY%LSNV) THEN
        ...
        ELSE
          PGZ0H(JLON) = SQRT((1.0_JPRB - PNEIJV(JLON))*PGZ0HF(JLON)**2 + PNEIJV(JLON)*(Z0CR**2 + (PGZ0RLF(JLON)*PGZ0HF(JLON) /  &
          & PGZ0F(JLON))**2))
        END IF
      ELSE
        IF (YDPHY%LZ0HSREL) THEN
          PGZ0H(JLON) = PGZ0HF(JLON) + (Z0CR*YDCLI%STHER - PGZ0HF(JLON))*PLSM(JLON)*PSNS(JLON) / (PSNS(JLON) +  &
          & YDPHY1%WCRIN*(1.0_JPRB + ZUZ0CN*PGZ0HF(JLON)))
        ELSE
          PGZ0H(JLON) = PGZ0HF(JLON) + (Z0CR - PGZ0HF(JLON))*PLSM(JLON)*PSNS(JLON) / (PSNS(JLON) + YDPHY1%WCRIN*(1.0_JPRB +  &
          & ZUZ0CN*PGZ0HF(JLON)))
        END IF
      END IF
    ELSE
    ...
    ELSE
      PGZ0H(JLON) = MIN(PGZ0(JLON), PLSM(JLON)*PGZ0H(JLON) + (1.0_JPRB - PLSM(JLON))*PGZ0(JLON)*YDPHY1%RZHZ0M)
    END IF
  ...
  IF (YDPHY%LSOLV) THEN
    ...
    END IF
    IF (YDPHY%LSNV) THEN
      PHU(JLON) = 1.0_JPRB + PLSM(JLON)*(1.0_JPRB - PNEIJG(JLON))*(0.5_JPRB*(1.0_JPRB - COS(YDCST%RPI*MIN(ZWS / ZWFC, 1.0_JPRB))) &
      &  - 1.0_JPRB)
      PHU(JLON) = MAX(PHU(JLON), MIN(PQ(JLON, KLEV) / PQSATS(JLON), 1.0_JPRB))
      PQS(JLON) = (PVEG(JLON)*PHV(JLON) + PVEG0(JLON)*PNEIJV(JLON) + (1.0_JPRB - PVEG0(JLON))*PHU(JLON))*PQSATS(JLON) + (1.0_JPRB &
      &  - PHV(JLON))*PVEG(JLON)*PQ(JLON, KLEV)
    ELSE
      PHU(JLON) = 1.0_JPRB + PLSM(JLON)*(1.0_JPRB - PNEIJ(JLON))*(0.5_JPRB*(1.0_JPRB - COS(YDCST%RPI*MIN(ZWS / ZWFC, 1.0_JPRB)))  &
      & - 1.0_JPRB)
      PHU(JLON) = MAX(PHU(JLON), MIN(PQ(JLON, KLEV) / PQSATS(JLON), 1.0_JPRB))
      PQS(JLON) = (PVEG(JLON)*PHV(JLON) + (1.0_JPRB - PVEG(JLON))*PHU(JLON))*PQSATS(JLON) + (1.0_JPRB - PHV(JLON))*PVEG(JLON) &
      & *PQ(JLON, KLEV)
    END IF
  ELSE
    PHU(JLON) =  &
    & 1.0_JPRB + PLSM(JLON)*(1.0_JPRB - PNEIJ(JLON))*(0.5_JPRB*(1.0_JPRB - COS(YDCST%RPI*PWS(JLON) / YDPHY1%WSMX)) - 1.0_JPRB)
  ...
  IF (YDPHY%LZ0HSREL .and. YDPHY%LSNV) THEN
    ZGZ0MR = PLSM(JLON)*SQRT(MAX(ZGZ0N**2, PGZ0(JLON)**2 - PGZ0RLF(JLON)**2)) + (1.0_JPRB - PLSM(JLON))*PGZ0(JLON)
  ...
  PRTI(JLON) = 2.0_JPRB / (PR(JLON, KLEV)*PT(JLON, KLEV) + YDCST%RKAPPA*ZDPHI(JLON) + PRS(JLON)*PTS(JLON))
  ZSTA = ZDPHI(JLON)*(PR(JLON, KLEV)*PT(JLON, KLEV) + YDCST%RKAPPA*ZDPHI(JLON) - PRS(JLON)*PTS(JLON))*PRTI(JLON)
  ZSTABV = ZSTA
  ...
  IF (YDPHY%LRRGUST) THEN
    ...
    ZRRCOR = SQRT(1.0_JPRB + ((((ZFP / (ZFP + YDPHY0%RRSCALE))**YDPHY0%RRGAMMA)*YDPHY0%UTILGUST)**2) / (PCD(JLON)*ZU**2))
  ...
  IF (LDHMT) THEN
    ...
    IF (YDPHY2%LRAFTUR) THEN
      ZSCRAF = (PCD(JLON)*(PU(JLON, KLEV)**2 + PV(JLON, KLEV)**2) / (PUCLS(JLON)**2 + PVCLS(JLON)**2 +  &
      & (YDPHY0%GCISMIN*PDPHIV(JLON) / YDCST%RG)**2)) / (1.0_JPRB + (LOG(1.0_JPRB + PDPHIV(JLON) / YDPHY2%GZ0RAF) / LOG(1.0_JPRB  &
      & + PDPHIV(JLON) / PGZ0(JLON)))**2)
