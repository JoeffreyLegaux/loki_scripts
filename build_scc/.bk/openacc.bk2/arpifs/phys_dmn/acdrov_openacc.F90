...
#include "stack.h"
  ...
  ZFLDT = YDML_PHY_MF%YRPHY2%TSPHY*(ZVEG*(PFPLSL(JLON, KLEV) + PFPLCL(JLON, KLEV)) + PFEVV(JLON) - PFTR(JLON))
  PRUISL(JLON) = ZITSP*PLSM(JLON)*(ZFLDT - MIN(PWLMX(JLON) - PWL(JLON), MAX(-PWL(JLON), ZFLDT)))
  ZBFLO = (1.0_JPRB - ZVEG)*(PFPLSL(JLON, KLEV) + PFPLCL(JLON, KLEV)) + PRUISL(JLON) + PFEVL(JLON) - PFEVV(JLON)
  ...
  IF (YDML_PHY_MF%YRPHY%LNEIGE) THEN
    ...
    ZTSP = PTS(JLON) + ZPCTDT*(PFRSO(JLON, KLEV) + PFRTH(JLON, KLEV) + PFCS(JLON) + PFCLL(JLON) + PFCLN(JLON) - PFCHSP(JLON))
    ...
    IF (YDML_PHY_MF%YRPHY%LSNV) THEN
      ...
      PFONTE(JLON) = MAX(0.0_JPRB, (ZTF - (PLSM(JLON)*YDCST%RTT + (1.0_JPRB - PLSM(JLON))*YDML_PHY_MF%YRPHY1%TMERGL)) /  &
    ...
    ELSE
      PFONTE(JLON) = PLSM(JLON)*MIN(ZSNSP, MAX(0.0_JPRB, (ZTSP - (PLSM(JLON)*YDCST%RTT + (1.0_JPRB - PLSM(JLON)) &
    ...
    IF (YDML_PHY_MF%YRPHY%LFGEL) THEN
      ...
      !  GEL DE SURFACE.
      ZZK = PLSM(JLON)*YDML_PHY_MF%YRPHY1%GCGELS*MAX(0.0_JPRB, 1.0_JPRB - PVEG(JLON) / YDML_PHY_MF%YRPHY1%GVEGMXS)*MAX(0.0_JPRB,  &
      & 1.0_JPRB - PLAI(JLON) / YDML_PHY_MF%YRPHY1%GLAIMXS)*MAX(0.0_JPRB, 1.0_JPRB - PNEIJ(JLON) / YDML_PHY_MF%YRPHY1%GNEIMXS)
      ...
      ZZK = PLSM(JLON)*YDML_PHY_MF%YRPHY1%GCGEL*MAX(0.0_JPRB, 1.0_JPRB - PVEG(JLON) / YDML_PHY_MF%YRPHY1%GVEGMX)*MAX(0.0_JPRB,  &
      & 1.0_JPRB - PLAI(JLON) / YDML_PHY_MF%YRPHY1%GLAIMX)*MAX(0.0_JPRB, 1.0_JPRB - PNEIJ(JLON) / YDML_PHY_MF%YRPHY1%GNEIMX)
  ...
  PRUISP(JLON) =  &
  & ZITSP*PLSM(JLON)*(ZFLDT - MIN(ZWPMX - PWP(JLON), MAX(-PWP(JLON), ZFLDT))) + PC3(JLON)*ZITS*MAX(0.0_JPRB, PWP(JLON) - ZWFC)
