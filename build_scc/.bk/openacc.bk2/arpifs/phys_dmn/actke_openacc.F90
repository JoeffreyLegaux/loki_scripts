...
#include "aclender_openacc.intfb.h"
  ...
  ELSE
    ...
    END IF
  END IF
  IF (KIND (ZKCLS) == 8) THEN
    alloc8 (ZKCLS)
  ELSE
    IF (KIND (ZKCLS) == 4) THEN
      alloc4 (ZKCLS)
    ELSE
      STOP 1
    END IF
  END IF
  IF (KIND (ZECTCLS) == 8) THEN
    alloc8 (ZECTCLS)
  ELSE
    IF (KIND (ZECTCLS) == 4) THEN
      alloc4 (ZECTCLS)
    ELSE
      STOP 1
    END IF
  END IF
  IF (KIND (ZTABHL) == 8) THEN
    alloc8 (ZTABHL)
  ELSE
    IF (KIND (ZTABHL) == 4) THEN
      alloc4 (ZTABHL)
    ELSE
      STOP 1
    END IF
  END IF
  IF (KIND (ZTABFL) == 8) THEN
    alloc8 (ZTABFL)
  ELSE
    IF (KIND (ZTABFL) == 4) THEN
      alloc4 (ZTABFL)
    ELSE
      STOP 1
    END IF
